...
const makeChunks = (input) => {
    let vals = input.split(/[\r\n]/).map(Number)
    let chunks = []

    while(vals.length>0){
        let newChunk = [vals.shift()]
        
        while(vals[0]<newChunk.at(-1)){
            newChunk.push(vals.shift())
        }

        let rObj = { "groupDiff":newChunk[0]-newChunk.at(-1),"vals":newChunk }
        rObj.before = !chunks?.at(-1) ? Number.MAX_SAFE_INTEGER : newChunk[0]-chunks?.at(-1).vals.at(-1)
        rObj.after = !vals[0] ? Number.MAX_SAFE_INTEGER : vals[0]-newChunk.at(-1)
...
const first = (input) => {
    ...
    while(ducks.length>0){
        ...
        while(thisObj.avg>ducks[0]){
            console.log('rolling avg larger ',thisObj.vals,ducks[0])
        ...
        while(processed.length>0 && processed.at(-1).avg>thisObj.avg){
            console.log('prev avg larger ',thisObj.avg,thisObj.vals, processed.at(-1).avg,processed.at(-1).vals)
